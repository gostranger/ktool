 ...
 *
 * The methods are all based on calls to the SDK methods, these are just
...
public class helper {
	 ...
	 * @param mode          {@link IfExists} {@code enum} specifying whether to
	 *                      overwite or throw an exception if the file already
	 *                      exists
	 ...
	 * @param mode     {@link IfExists} {@code enum} specifying whether to overwite
	 *                 or throw an exception if the file already exists
	 * @throws IOException thrown on error
	 */
	...
	public void upload(String filename, byte[] contents, IfExists mode) throws IOException {
		...
		if (contents.length <= 4 * 1024 * 1024) { // if less than 4MB, then do a direct CREATE in a single operation
			...
			opts.retryPolicy = overwrite ? new ExponentialBackoffPolicy() : new ExponentialBackoffPolicy();
			OperationResponse resp = new OperationResponse();
			Core.create(filename, overwrite, null, contents, 0, contents.length, null, null, true, SyncFlag.CLOSE,
					client, opts, resp);
			if (!resp.successful) {
